...
import { generateDemoForScene, type GenerationFormState } from "@/app/actions";
...
export function DemoGenerationForm({ scene }: { scene: string }) {
  const [state, action, pending] = useActionState(generateDemoForScene, initialState);

  return (
    <form action={action} className="rounded-lg border border-ink/10 bg-white p-5 shadow-sm">
      <input name="scene" type="hidden" value={scene} />
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="项目名称" name="projectName" placeholder="例如无痛肩颈根治体验、三伏贴、艾灸" />
        <Field label="目标客户" name="targetCustomer" placeholder="例如长期伏案的上班族、宝妈、儿童家长" />
        ...
        <Field label="补充信息" name="extraInfo" placeholder="例如真实克制，适合中医馆发布，不要夸大疗效" required={false} />
      </div>
