...
describe("mock data store", () => {
  it("logs in seeded admin and customer accounts by phone and password", () => {
    ...
    expect(store.login("13900000001", "trial123")).toMatchObject({
      role: "user",
      planName: "free_trial",
    });
    expect(store.login("13900000001", "wrong-password")).toBeNull();
  });
  ...
  it("records trial applications and status changes", () => {
    const store = createMockStore();
  ...
  it("records generation results and supports copy and note updates", () => {
    const store = createMockStore();
    const user = store.login("13900000001", "trial123");
