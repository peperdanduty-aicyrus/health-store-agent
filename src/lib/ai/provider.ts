...
function buildMockWorkbenchBody(type: WorkbenchGenerationType, input: WorkbenchInput) {
  if (type === "mealbox_video") {
    ...
    return {
      ...
      openingShotIdeas: ["医院食堂饭菜扫一眼，字幕写今天午休又看店。", "拍工牌、饭盒、手机放在桌边，节奏像日常记录。"],
      storyboard: [
        {
          duration: "2-3秒",
          shot: "镜头1",
...
function buildMockPromotionBody(input: WorkbenchInput) {
  ...
  if (platform === "闲鱼") {
    return {
      xianyuChatReplies: [`可以发我店铺页面，我先按 ${product} 看几个明显问题。`, "我主要看标题、主图、团单和客户下一步怎么联系。", "不承诺订单，只帮你先看页面问题。", `如果只是想先了解，可以先做一个${product}。`, "你把行业和页面发来，我按本地门店视角看。"],
      xianyuDetails: [
        `适合${target}先看看线上页面哪里有问题。${product}主要看客户第一眼是否看懂主推项目、页面有没有信任感、团单和承接路径是否清楚。`,
  ...
  return {
    ...
    momentsHumanPosts: [
      ...
      "今天午休看了几个本地门店页面，第一眼信息太散。客户不是不想了解，是不知道先看哪里。",
